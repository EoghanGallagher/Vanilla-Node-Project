   ...
   _data.read('tokens', token, function(err, tokenData){
     if(!err && tokenData)
     {
      ...
      _data.read('users', userPhone, function(err, userData){
        if(!err && userData){
          var userChecks = typeof(userData.checks) == 'object' && userDats.checks instanceof Array ? userData.checks : [];
          ...
          if(userChecks.length < config.maxChecks){
            ...
            callback(400 , {'Error' : `User already has maximum number of checks which is ${config.maxChecks}`});
          }


        } else {
            callback(403,{"Error" : "Missing user"});
        }
      });
